document.addEventListener('DOMContentLoaded', () => {
  ...
  btnSubmit.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const base = data.base_probs || {};
      const dbg = `inputs: ${Object.entries(inputs).map(([k,v])=>`${k}=${Number(v).toFixed(2)}`).join(', ')}; base: ${Object.entries(base).map(([k,v])=>`${k}=${Number(v).toFixed(3)}`).join(', ')}`;
      if (elDebug) elDebug.textContent = dbg;
