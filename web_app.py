...
# 备用交互页：不依赖模板目录，直接内联一个可输入并提交到 /predict 的页面
@app.get('/form')
def form_page():
    try:
        ...
        html = (
            ...
            "<title>在线预测</title><style>body{font-family:system-ui,Segoe UI,Arial;max-width:960px;margin:40px auto;line-height:1.6}label{display:block;margin-top:8px;color:#555}input{width:100%;box-sizing:border-box;padding:6px;margin-top:4px}</style></head><body>"
            "<h1>在线预测</h1><p>健康检查：<a href='/health'>/health</a></p>"
            "<div id='form'></div><p><button id='btnSample'>用示例填充</button> <button id='btnPredict'>提交预测</button></p>"
            "<div id='result' style='border:1px solid #ddd;border-radius:8px;padding:12px;margin-top:12px;display:none'></div>"
            "<script>"
            f"const sample = {sample_json}; const keys = Object.keys(sample||{}); const inputs={}; const formDiv=document.getElementById('form');"
            "keys.forEach(function(k){var row=document.createElement('div');var label=document.createElement('label');label.textContent=k;var input=document.createElement('input');input.type='number';input.step='any';row.appendChild(label);row.appendChild(input);formDiv.appendChild(row);inputs[k]=input;});"
            "document.getElementById('btnSample').onclick=function(){keys.forEach(function(k){inputs[k].value=sample[k];});};"
            "function payload(){var obj={}; keys.forEach(function(k){var v=inputs[k].value; obj[k]=v===''?null:Number(v);}); return {instances:[obj]};}"
            "document.getElementById('btnPredict').onclick=async function(){var result=document.getElementById('result'); result.style.display='block'; result.textContent='预测中...'; try{var resp=await fetch('/predict',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload())}); var data=await resp.json(); if(!resp.ok){result.textContent='错误：'+(data.error||'预测失败'); return;} var p=Array.isArray(data.proba)?data.proba[0]:data.proba; var lbl=Array.isArray(data.label)?data.label[0]:data.label; result.innerHTML='<b>预测概率：</b> '+p+'<br/><b>预测标签：</b> '+lbl;}catch(e){result.textContent='网络错误或服务未启动：'+e;}};"
            "</script></body></html>"
        )
        return html, 200, {"Content-Type": "text/html; charset=utf-8"}
    except Exception as e:
