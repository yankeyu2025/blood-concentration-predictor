...
# --- 模型加载 ---
# 定义模型和相关文件的目录（优先使用 __file__ 相对路径；支持环境变量）
# 这样无论是在本地还是 Render 环境，都能正确定位到位于同目录下的“集成模型”文件夹。
...
if not MODEL_DIR:
    # 依次尝试常见位置：当前文件同级的“集成模型”、工作目录下的“集成模型”、Render 默认路径
    candidate_dirs = [
    ...
    # 若仍未找到，默认退回到 BASE_DIR/集成模型（即使不存在，也会在加载时给出清晰错误）
    if not MODEL_DIR:
        MODEL_DIR = os.path.join(BASE_DIR, '集成模型')
