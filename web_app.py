...
def predict():
    ...
    try:
        ...
        # Create a DataFrame with all features expected by the scaler
        ...
        # Map our input features to the expected feature names
        feature_mapping = {
            'Daily dose（g）': 'Daily dose(g)',  # Note: different parentheses
            'CLCR': 'CLCR',
            'ALP(U/L)': 'ALP(U/L)',
            'GGT(U/L)': 'GGT(U/L)',
            'Na(mmol/L)': 'Na(mmol/L)',
        ...
        for input_feature, scaler_feature in feature_mapping.items():
            if scaler_feature in all_features:
                full_input_data[scaler_feature] = float(data[input_feature])
            else:
        ...
        # Based on Boruta feature selection results
