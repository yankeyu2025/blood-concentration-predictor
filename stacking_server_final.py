...
# -*- coding: utf-8 -*-
"""
Updated Stacking Web Server for Final Models
Uses the new Top 3 models: Random Forest, SVM, XGBoost with Stacking
"""
...
def get_texts():
    if get_language() == 'en':
        return {
            ...
            'subtitle': 'Powered by Stacking Ensemble (RF + SVM + XGBoost)',
            ...
            'footer': 'CLCR Prediction · Stacking Ensemble',
            ...
            'usage_reminder_items': [
                ...
                'Model: Stacking Ensemble (Random Forest + SVM + XGBoost)'
            ],
            'feature_units': {
                'CLCR': 'CLCR (mL/min)',
                'height': 'Height (cm)',
        ...
        return {
            'title': '碳酸锂血药浓度预测系统',
            'subtitle': '基于Stacking集成模型（随机森林+支持向量机+XGBoost）',
            'tips': '请填写患者特征后进行预测',
            ...
            'usage_reminder_items': [
                '本系统仅供临床参考，不能替代医疗判断',
                '请确保输入单位与要求一致',
                '模型：Stacking集成（随机森林+支持向量机+XGBoost）'
            ],
            'feature_units': {
                'CLCR': 'CLCR（mL/min）',
                'height': '身高（cm）',
...
if not MODEL_DIR:
    candidates = [
        os.path.join(BASE_DIR, '3_最终评估与报告'),  # 优先使用最终评估目录
    ...
    for d in candidates:
        if os.path.exists(d):
            # 检查是否包含新的最终模型文件
            final_models = ['随机森林_最终训练模型.pkl', '支持向量机_最终训练模型.pkl', 'XGBoost_最终训练模型.pkl']
            if all(os.path.exists(os.path.join(d, model)) for model in final_models):
                MODEL_DIR = d
                break
            elif os.path.exists(d):  # 如果没有最终模型，使用第一个存在的目录
                MODEL_DIR = d
                break
    if not MODEL_DIR:
        MODEL_DIR = os.path.join(BASE_DIR, '3_最终评估与报告')
...
def load_model():
    try:
        model = {}
        # 基模型（新的Top 3模型）
        model['rf'] = joblib.load(os.path.join(MODEL_DIR, '随机森林_最终训练模型.pkl'))
        model['svm'] = joblib.load(os.path.join(MODEL_DIR, '支持向量机_最终训练模型.pkl'))
        model['xgb'] = joblib.load(os.path.join(MODEL_DIR, 'XGBoost_最终训练模型.pkl'))
        logging.info('Base models loaded (RF, SVM, XGBoost).')
        ...
        if not meta_path:
            raise FileNotFoundError('缺少Stacking元学习器：Stacking_元学习器.pkl')
        model['meta'] = joblib.load(meta_path)
        logging.info(f'Meta learner loaded from {os.path.basename(os.path.dirname(meta_path))}')
...
def predict():
    try:
        ...
        # 新的基模型预测
        p_rf  = float(ensemble_model['rf'].predict_proba(Xs)[:, 1][0])
        p_svm = float(ensemble_model['svm'].predict_proba(Xs)[:, 1][0])
        p_xgb = float(ensemble_model['xgb'].predict_proba(Xs)[:, 1][0])

        cfg = ensemble_model.get('stacking_cfg')
        probs_map = {'随机森林': p_rf, '支持向量机': p_svm, 'XGBoost': p_xgb}
        if cfg and isinstance(cfg, dict):
            ...
            if order and isinstance(order, list) and all(n in probs_map for n in order):
                ...
                meta_X = np.array([[p_rf, p_svm, p_xgb]], dtype=float)
        else:
            meta_X = np.array([[p_rf, p_svm, p_xgb]], dtype=float)
        ...
        # 使用最优阈值（来自最终评估）
        threshold = 0.57  # 来自Stacking验证集的最佳阈值
        ...
        if instances:
            return jsonify({
                ...
                'threshold': threshold,
                'filled': filled,
                'model_info': 'Stacking Ensemble (RF+SVM+XGBoost)'
            })
        else:
            try:
                return render_template(
