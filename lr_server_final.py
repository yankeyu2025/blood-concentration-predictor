...
def get_texts():
    if get_language() == 'en':
        return {
            ...
            'footer': 'CLCR Prediction · Logistic Regression',
            'feature_units': {
                'height': 'Height (cm)',
                'weight': 'Weight (kg)',
    ...
    return {
        ...
        'prob_label': '阳性概率',
        'risk_label': '风险等级',
        'footer': 'CLCR 预测系统 · Logistic Regression',
        'feature_units': {
            'height': '身高（cm）',
            'weight': '体重（kg）',
            'Daily dose(g)': '每日剂量（g）',
...
def load_model():
    try:
        ...
        # 逻辑回归模型
        lr_candidates = [
            os.path.join(MODEL_DIR, '逻辑回归_最终训练模型.pkl'),
            os.path.join(BASE_DIR, '3_最终评估与报告', '逻辑回归_最终训练模型.pkl')
        ]
        lr_path = next((p for p in lr_candidates if os.path.exists(p)), None)
        if not lr_path:
            raise FileNotFoundError('缺少 逻辑回归_最终训练模型.pkl')
        model['lr'] = joblib.load(lr_path)
        model['lr_path'] = lr_path
...
def info():
    return jsonify({
        ...
        'lr_path': ensemble_model.get('lr_path'),
...
def models_info():
    ...
    try:
        ...
        info['Logistic Regression'] = {'has_feature_names_in': False, 'feature_count': None, 'feature_names_in': None}
    return jsonify(info)
...
def predict():
    try:
        ...
        if base_order:
            ...
            X_eval = Xs[:, cols] if len(cols) == len(base_order) else Xs
        else:
            X_eval = Xs
